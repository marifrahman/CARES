...
class DX_Auth {
    ...
    //Get user's particular profile info
    function get_profile_photo() {
        $photo = $this->get_profile_info("usr_photo");
        return (isset($photo[0]["usr_photo"]) && $photo[0]["usr_photo"] != "" )? $photo[0]["usr_photo"] : 'profile.jpg';
    }

    //Get user's particular profile info
    function get_profile_info($info_required) {
        $this->ci->load->model("dx_auth/user_profile", "user_profile");
        return $this->ci->user_profile->get_profile_field($this->get_user_id(), $info_required);
    }
    ...
    function register($username, $password, $email, $profile_data = array()) {
        // Load Models
        $this->ci->load->model('dx_auth/users', 'users');
        $this->ci->load->model('dx_auth/user_temp', 'user_temp');
        ...
        if ($this->ci->config->item('DX_email_activation')) {
            ...
            // Trigger event
            $this->ci->dx_auth_event->user_activated($this->ci->db->insert_id(), $profile_data);
        }
    ...
    function change_user_info($user_info) {

        $result = FALSE;
        // Crypt and encode new password
        $new_pass = crypt($this->_encode($user_info["new_pwd"]));
        // Replace old password with new password

        if ($this->ci->users->change_password($user_info["user_id"], $new_pass)) {
            // Trigger event
            //$this->ci->dx_auth_event->user_changed_password($this->ci->session->userdata('DX_user_id'), $new_pass);
            $result = TRUE;
        }
