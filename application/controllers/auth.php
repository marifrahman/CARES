...
class Auth extends CI_Controller {
    ...
    function username_check($username) {
        ...
        if (!$result) {
            $this->form_validation->set_message('username_check', '<font style="color:red;">Username already exist. Please choose another username.</font>');
        }
    ...
    function email_check($email) {
        ...
        if (!$result) {
            $this->form_validation->set_message('email_check', '<font style="color:red;">Email is already used by another user. Please choose another email address.</font>');
        }
    ...
    function login() {
        ...
        if (!$this->dx_auth->is_logged_in()) {
            ...
            $data['auth_message'] = 'You are already logged in.<a href=' . site_url() . '/auth/logout>Logout</a>';

            //load user activity
            //$data["useractivity"] = $this->loadUserActivity();
    ...
    function logout() {
        ...
        //$data['auth_message'] = 'You have been logged out.<a href=' . site_url() . '/auth/login>Login</a>';
        //$this->load->view($this->dx_auth->logout_view, $data);
        $this->load->view($this->dx_auth->logout_view);
    }

    function _uploadPhoto($photo_file) {


        $this->load->config("saf_config");

        $this->load->model("upload_model");
        $this->upload_model->gallery_path = realpath(APPPATH . "../" . $this->config->item("usr_photo_dir"));
        $image_data = $this->upload_model->do_upload();
        return (isset($image_data["file_name"])) ? $image_data["file_name"] : "";
    }

    /*
     * Modified register function so that admin can Create users 
     */
    ...
    function createUser() {
        ...
        if ($this->dx_auth->is_admin()) {
            ...
            $data["msg"] = "";
            //$this->load->model("safmodel");
            //$data["provincelist"] = $this->safmodel->loadProvince();
            ...
            if ($this->input->post("submit")) {
                ...
                //$profile_data["employee_id"] = $this->input->post("employee_id");
                //$profile_data["region"] = $this->input->post("user_region");
                $profile_data["role_id"] = $this->input->post("user_role");
                
                //$user_photo = $this->_uploadPhoto($this->input->post("userfile"));
                
                //$profile_data["usr_photo"] = (is_array($user_photo))?$user_photo["file_name"]:'';
                // Run form validation and register user if it's pass the validation
                if ($val->run() AND $this->dx_auth->register($val->set_value('username'), $val->set_value('password'), $val->set_value('email'), $profile_data)) {

                    $data["successmsg"] = "You have successfully created the user";
            ...
            /* $data['msg'] = "<div class=\"alert alert_red\"><img height=\"24\" width=\"24\" src=\"" . base_url('assets/images/icons/small/white/Alert.png') . "\">
              <strong> Sorry, You have to be logged in as admin to create an user !!</strong> !</div>";
              $this->load->view($this->dx_auth->login_view, $data); */
            redirect('', "location");
        }
    }

    function change_user_info($user_id) {
        
        if (isset($user_id) && ($this->dx_auth->is_admin() || $this->dx_auth->get_user_id() == $user_id)) {
            $data["msg"] = "";
            //$this->load->model("safmodel");
            //$data["provincelist"] = $this->safmodel->loadProvince();
            $this->load->model("dx_auth/users", "users");
            $user = $this->users->get_user_by_id($user_id)->result();
            $data["user"] = $user[0];
            ...
            if($this->input->post("submit"))
            {
                ...
                $user_info["new_pwd"] = $this->input->post("password");
                if($this->dx_auth->change_user_info($user_info))
                    $data["successmsg"]= " You have successfully updates the user info .";
                else
                    $data["errormsg"] = "Opps, got some problem !";
                    
            }
            
            //var_dump($user); die("");
            
            $this->load->view("auth/change_info_form", $data);
        }
        else
            redirect(site_url(), "");
    }
    ...
    function loadUserActivity() {
        ...
        if ($this->dx_auth->is_admin()) {
            $this->load->library("A5ActivityLogger");
            return $this->a5activitylogger->pullActivity();
        }
        return false;
    }
